...
const CALENDLY_URL = 'https://calendly.com/loremex/intro'
...
function buildEmailHtml(params: SendScorecardEmailParams, baseUrl: string): string {
            ...
            <table cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
              <tr>
                <td style="background:#2563EB;border-radius:8px;padding:12px 24px;">
                  <a href="${CALENDLY_URL}" style="font-size:14px;font-weight:600;color:#ffffff;text-decoration:none;">
...
export async function sendScorecardEmail(params: SendScorecardEmailParams): Promise<SendEmailResult> {
  ...
  const replyTo = process.env.RESEND_REPLY_TO ?? 'hello@loremex.ai'
  const baseUrl = process.env.VITE_BASE_URL ?? 'http://localhost:5173'

  return withRetry(async () => {
    const resend = getResend()
    const html = buildEmailHtml(params, baseUrl)
